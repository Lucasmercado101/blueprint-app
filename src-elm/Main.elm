    ...
    let
        ...
        thereIsALineInsidePrevRoom =
            (nextRoom :: nextRooms)
                ...
                |> List.filter (\l -> y2 l < prevRoom.y1)
                |> List.foldl
                    (\next curr ->
                        case curr of
                            Just val ->
...
getLeftYSegmentsHelper prevRoom ((Nonempty nextRoom nextRooms) as allRooms) =
    let
        ...
        thereIsALineInsidePrevRoom =
            (nextRoom :: nextRooms)
                |> List.filter (not << Rect.eq prevRoom)
                |> List.filter (\l -> x2 l < prevRoom.x1)
                |> List.foldl
                    (\next curr ->
                        case curr of
                            Just val ->
        ...
        bottomAndLeftMostFoldl : List Rectangle -> Maybe Rectangle
        bottomAndLeftMostFoldl =
            List.foldl
                (\next curr ->
                    case curr of
                        Just val ->
                            if y2 next >= y2 val then
                                if y2 next == y2 val then
                                    if next.x1 < val.x1 then
                                        Just next
        ...
        closestLineAbove : Maybe Rectangle
        closestLineAbove =
            (nextRoom :: nextRooms)
                |> List.filter (\l -> y2 l < prevRoom.y1)
                |> bottomAndLeftMostFoldl
    ...
    case insideOfAnotherRoom of
        Just roomImInsideOf ->
            ListNE.cons (LineSegment ( prevRoom.y1, prevRoom.height ))
                (getLeftYSegmentsHelper
                    { roomImInsideOf
                        | y1 = roomImInsideOf.y1
                        , height = prevRoom.y1 - roomImInsideOf.y1
                    }
                    allRooms
                )
        ...
        Nothing ->
            case thereIsALineInsidePrevRoom of
                Just lineInsidePrevRoom ->
                    if lineInsidePrevRoom.y1 == prevRoom.y1 then
                        ListNE.cons (LineSegment ( y2 lineInsidePrevRoom, y2 prevRoom - y2 lineInsidePrevRoom )) (getLeftYSegmentsHelper lineInsidePrevRoom allRooms)

                    else
                        ListNE.append
                            (Nonempty (LineSegment ( y2 lineInsidePrevRoom, y2 prevRoom - y2 lineInsidePrevRoom )) [])
                    ...
                    case linePartiallyInsideTopLeft of
                        Just lineInsideTopLeft ->
                            if y2 lineInsideTopLeft == prevRoom.y1 then
                                ListNE.cons (LineSegment ( prevRoom.y1, prevRoom.height )) (getLeftYSegmentsHelper lineInsideTopLeft allRooms)
                        ...
                        Nothing ->
                            let
                                linePartiallyInsideTopRight =
                                    (nextRoom :: nextRooms)
                                        |> List.filter (\l -> x2 prevRoom < l.x1 && y2 l >= prevRoom.y1 && l.y1 < prevRoom.y1)
                                        |> bottomAndLeftMostFoldl
                            in
                            case linePartiallyInsideTopRight of
                                Just lineInsideTopRight ->
                                    if y2 lineInsideTopRight == prevRoom.y1 then
                                        ListNE.cons (LineSegment ( prevRoom.y1, prevRoom.height )) (getLeftYSegmentsHelper lineInsideTopRight allRooms)

                                    else
                                        ListNE.append
                                            (Nonempty (LineSegment ( y2 lineInsideTopRight, y2 prevRoom - y2 lineInsideTopRight )) [])
                                            (getLeftYSegmentsHelper lineInsideTopRight allRooms)
